...
def load_images(
    ...
    base_path = os.path.join(
        os.path.dirname(sys.argv[0]), "assets", "images", folder
    )
    images = []
...
def load_audio(path: str, volume: float = 1.0) -> pg.Sound:
    full_path = os.path.join(
        os.path.dirname(sys.argv[0]), "assets", "audio", path
    )
...
def get_random_position(
    ...
    while True:
        x = random.randint(
            max_rect.left + size[0], max_rect.right - size[0]
        )
        y = random.randint(
            max_rect.top + size[1], max_rect.bottom - size[1]
        )
